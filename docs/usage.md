...
すぐに反映させたいときは、ヘッダー右上の ⟳ ボタン（再スキャン）を押すとその場で再スキャンします。
...
![ファイル名で表示](./screenshots/label-mode.png)

ボタンには現在のモードが出ます。押すたび（または `t` キー）に切り替わり、選択はブラウザ（localStorage）に
保存されるので次回起動時も維持されます。検索はどちらのモードでもタイトル・ファイル名の両方を対象にします。
