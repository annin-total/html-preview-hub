...
5. **LaTeX プレビュー**: `.tex` を開くと `POST /api/tex/compile` が呼ばれ、`tex.compile_tex()` がエンジンを判定してコンパイルし、生成物をソース内容とエンジン名のハッシュ（fingerprint）でキャッシュします。フロントエンドは返ってきた `pdfUrl`（`GET /api/tex/pdf`）を iframe に読み込みます。
