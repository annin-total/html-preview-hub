...
export const api = {
  ...
  watch: (revision, signal) =>
    request(`/api/index/watch?revision=${revision}`, { signal }),
  ...
  source: (fileId) =>
    request(`/api/source?fileId=${encodeURIComponent(fileId)}`),
  texStatus: () => request("/api/tex/status"),
  /** コンパイルを開始する。時間がかかる場合は status: "running" が返る。 */
  texCompile: (fileId, force = false) =>
    post("/api/tex/compile", { fileId, force }),
  /** バックグラウンドで走っているコンパイルの状態を取得する。 */
  texJob: (fileId) => request(`/api/tex/job?fileId=${encodeURIComponent(fileId)}`),
