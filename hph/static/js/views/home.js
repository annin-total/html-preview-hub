...
export function createHomeView({
  ...
  function renderKindChips() {
    ...
    const present = KINDS.filter(
      (kind) => kind.id === "all" || counts.get(kind.id),
    );
    if (present.length <= 2) {
    ...
    const card = el(
      ...
      [
        el("header", { class: "card__head" }, [
          ...
          el("h3", {
            class: "card__name",
            html: highlight(folder.name, terms),
          }),
          ...
          files.map((file) =>
            el(
              ...
              [
                ...
                file.kind !== "html"
                  ? el("em", { class: "kind-badge", text: file.kind })
                  : null,
              ],
            ),
          ),
  ...
  function appendChunk() {
    ...
    for (let i = rendered; i < end; i += 1)
      fragment.append(buildCard(folders[i]));
